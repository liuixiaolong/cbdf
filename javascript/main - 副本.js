...
function ready(){
	...
	//pagesSwiper.swipeTo(6,0);   //======================testing 
...
function transform(){	
	$("#preview").css({"-webkit-transform":'translate3d(' + offx + ',' + offy + ',0)'+' scale(' + targetScale + ')'+' rotate('+targetRotate+'deg)'});
	$("#preview1").css({"-webkit-transform":'translate3d(' + offx + ',' + offy + ',0)'+' scale(' + targetScale + ')'+' rotate('+targetRotate+'deg)'});
	$("#smallPhotoBox").css({"-webkit-transform":'translate3d(' + offx + ',' + offy + ',0)'+' scale(' + targetScale + ')'+' rotate('+targetRotate+'deg)'});
...
function savePhoto(){
		...
		html2canvas( $("#diyPhoto") ,{  		
				onrendered: function(canvas){					
					...
					$.post('uploadphoto.php',para,function(data){
						if(data!=null){
							if(data.result==0){
								alert(data.remark);
							}else{
							...
							//alert('照片上传成功！~');							
							$("#subLoadingBox").css({"display":"none"});
							var imgsrc = data.remark;
							$(".showPhotoBox").css({"display":"block"});
							$("#resultPhoto").attr("src",imgsrc);
							trackEvent( 'button', 'click', 'printPhoto-superUser');	
