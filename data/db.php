...
Class DB {
    ...
    public function __construct() {
        ...
        if($this->is_log){
        	...
        	$handle = fopen($db_config["logfilepath"]."dblog-".date('Y-m-d').".txt", "a+");
        	$this->handle=$handle;
        }        
    ...
    //执行事务
    public function exec_transaction($sqlarr=array()){
    	if(!is_array($sqlarr)){return;}
    	$isexec = false;
    	mysql_query("BEGIN");
    	$sqlqty=0;
    ...
    public function get_sql($sqlarray=array()){
    	if(!is_array($sqlarray)){return;}
    	$sql='select ';
        //读取数据列
        if (!empty($sqlarray['fields'])) {
    		$sql.=$sqlarray['fields'];
    	}else{
    		$sql.=' * ';
    	}
    	$sql.=' from '.$sqlarray['table'].' ';
    	//查询条件
